...
namespace MVVM_Essentials_Sample.Views.BasicTasks
{
    ...
    public sealed partial class BasicTasks : Page
    {
        public BasicTasks()
        {
            ...
            checkInternet();
            brdColor.Background = new SolidColorBrush(MVVM_Essentials_Lib.ConvertColor.ConvertStringToColor("0000FF"));
            txtRandomNumber.Text = MVVM_Essentials_Lib.CreateRandomNumber.IsAutoGenerated(15);
        }
        ...
        private void TxtColorName_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                if (txtColorName.Text.Length == 6)
                    brdColor.Background = new SolidColorBrush(MVVM_Essentials_Lib.ConvertColor.ConvertStringToColor(txtColorName.Text));
            }
            catch (Exception ex)
            {
                AlertBox.ShowBox("Error", ex.Message.ToString());
            }
        }

        private void TxtInitials_TextChanged(object sender, TextChangedEventArgs e)
        {
            
                if (txtColorName.Text.Length > 0)
                txtInit.Text = CreateInitials.getInitial(txtInitials.Text);
