...
namespace MVVM_Essentials_Lib.Camera
{
    public sealed partial class CameraControl
    {
        ...
        public async Task InitializeCameraAsync()
        {
            ...
            {
                throw new UnauthorizedAccessException("Camera_Exception_UnauthorizedAccess \nCamera Access \nError. Please provide access to app manually.\n\nSteps:\nStart  -> Settings  -> Privacy  ->  Camera", ex);
            }
            catch (NotSupportedException ex)
            {
                throw new NotSupportedException("Camera access restricted. \nPlease provide access to app manually.\n\nSteps:\nStart  -> Settings  -> Privacy  ->  Camera", ex);
            }
            catch
            {
                AlertBox.ShowBox("Warning", "Camera access restricted. \nPlease provide access to app manually.\n\nSteps:\nStart  -> Settings  -> Privacy  ->  Camera");
            }
        }
        ...
        public async Task<string> TakePhoto()
        {
            ...
            using (var stream = new InMemoryRandomAccessStream())
            {
                ...
                await StopPreviewAsync();
                _capturing = false;
                BitmapImage bitmapImage = new BitmapImage(new Uri(this.BaseUri, photo));
        ...
        private async Task SetPreviewRotationAsync()
        {
            try
            {
                ...
                int rotationDegrees = _displayOrientation.ToDegrees(Panel);
            ...
            {
                AlertBox.ShowBox("Warning", "Something went wrong");
            }
        }
        ...
        private async Task<string> ReencodeAndSavePhotoAsync(IRandomAccessStream stream, PhotoOrientation photoOrientation)
        {
            using (var inputStream = stream)
            {
                ...
                var file = await pictureFolder.CreateFileAsync("photo.jpeg", CreationCollisionOption.GenerateUniqueName);
        ...
        private async void CaptureRetryButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                await StartPreviewAsync();
